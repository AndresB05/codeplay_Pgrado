...
| **«Secure password change» quedó ENCENDIDO**, y no costó código: la reautenticación del cliente emite una sesión de segundos y al servidor le basta — `updateUser` respondió 200 y **no pidió nonce por correo**, comprobado por API y desde la pantalla de Ajustes con una cuenta nueva. La cautela que queda: lo que satisface al servidor es esa frescura, así que un cambio hecho desde una sesión vieja podría toparse con la exigencia del nonce, cuyo mensaje llega en inglés; ahí la respuesta es apagar el interruptor, no implementar el envío | Cerrado en el paso 13, tarea 10.1; ver `CONTEXT.md` §2.2 |
