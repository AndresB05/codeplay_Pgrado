...
`pantalla-compuesta-y-camara` (J6.3), `assets-de-tanteo` (J6.4) y
`reanudar-encuadre-y-felicitaciones` (14-sep-2026), más el J3, que sólo fijó el
formato por escrito.**
...
| `apps/web/src/components/dashboard/student/StudentLevelModule.tsx` | **Nuevo en el J7.1.** La pantalla de nivel, la primera de producto que monta el juego: lee la fila por su id, la comprueba con `openLevel`, y o monta el juego o enseña el rechazo del §7 **con palabras de niño**. Las instrucciones salen de la `narrative` de la fila. Posee la misma composición que el J6.3 ensayó en el laboratorio. **Desde `reanudar-encuadre-y-felicitaciones`**: mide dónde empieza la bandeja para el encuadre, **bloquea el lienzo y la caja** con el recorrido detenido, busca **el nivel siguiente del mundo** y abre **la ventana de felicitaciones** al llegar a la meta. **Ya no calca al laboratorio**: la caja de bloques va en la bandeja de abajo, repartidos a lo ancho en una fila (`BlockEditor` pasa a fila cuando su hueco es más ancho que alto, y sin «saltar» —mundo 1— los pinta a tamaño de lienzo), el lienzo ocupa la columna derecha con los botones debajo y su barra empieza y se queda arriba (`TopAnchoredMetrics` quita el margen que Blockly deja por encima), y las instrucciones son un único botón que empieza cerrado como «?» bajo «Vista inicial» y se abre y se cierra tocándolo |
...
| Google desde «Entrar» sin cuenta previa ya no crea un niño en silencio: la vuelta pregunta «Soy niño / Soy tutor» si el perfil tiene `is_role_declared = false`. Sin migración: la marca es de la 0018 y `set_my_role` ya permitía ese único cambio | `pages/AuthCallback/AuthCallback.tsx`, `User.isRoleDeclared` en `types/user.types.ts` y `profile.service.ts` |
| `ErrorBoundary` por encima del router: un error inesperado enseña un aviso con «Recargar» e «Ir al inicio» en vez de la pantalla en blanco. Es clase porque React no ofrece hook para `componentDidCatch` | `components/ui/ErrorBoundary.tsx`, montado en `App.tsx` |
| `vercel.json` para el despliegue provisional (paso 27.1): mismos `npm ci` y `npm run build` que el CI, salida `apps/web/dist` y **toda ruta reescrita a `index.html`**, sin la cual recargar o volver de Google a `/auth/callback` da 404 | `vercel.json` en la raíz |
| En «Mi salón» del niño las misiones pasan al final: podio y compañeros, que siempre están, van primero | `student/StudentClassroomModule.tsx` |
| Las pantallas de salón (las dos del tutor y la del niño) **releen cada 15 s** con la pestaña visible, sin indicador de carga: ranking, XP, racha y última actividad se mueven solos. Es sondeo porque el progreso no puede ir por Realtime (ver §2.7) | `hooks/useFreshClassrooms.ts` |
| Barra lateral plegable en los dos roles; plegada deja sólo los iconos, con `title`. La preferencia se recuerda en este navegador | `Sidebar/SidebarToggle.tsx`, `hooks/useSidebarCollapsed.ts`, `context/sidebar.helpers.ts`, montada en `Sidebar.tsx` y `teacher/TeacherSidebar.tsx` |
| La tabla de exploradores se ordena por «Más XP» (por defecto, el orden del podio), «Mayor racha» o «A-Z». La racha desempata por el orden del XP | `shared/StudentRosterTable.tsx`, `sortRoster` en `shared/podiumRanking.ts` |
| «Profesor» pasa a «Tutor» en toda la interfaz, también «Soy tutor» al elegir rol | 19 archivos de `apps/web/src` |
