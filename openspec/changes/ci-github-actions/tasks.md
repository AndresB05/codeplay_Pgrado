...
- [x] 1.3 Definir un único job sobre `ubuntu-latest` con los pasos `actions/checkout@v5` y `actions/setup-node@v5`, este último con `node-version: '22.17.1'` y `cache: 'npm'` (design, decisiones 3 y 8). Verificación: `grep -n "22.17.1" .github/workflows/ci.yml` devuelve la línea, y la ejecución no emite el aviso de deprecación de Node 20.
