...
contenido inicial de mundos y niveles existen en la base real, desde la
migración 0013 también las cuatro tablas del módulo de salones, desde la 0020
la asignación de misiones a un salón, desde la 0021 la publicación en vivo de
los cambios de tres de esas tablas, desde la 0022 **un segundo camino de
ingreso a un salón** —las dos funciones que generan y canjean una invitación— y
desde la 0034 y la 0035 **tres vistas que abren el progreso de un alumno a quien
lo tutela**, sin que las políticas de `user_progress` ni de `level_attempts`
hayan cambiado. Los requisitos describen un sistema en funcionamiento.

**Entrar a un salón ya no es una sola cosa.** Hasta la 0022 la pertenencia sólo
podía nacer de una solicitud aceptada; ahora nace también de una invitación
canjeada, y el esquema trata a las dos igual en lo que importa: ninguna la puede
componer el cliente, las dos escriben en una transacción y las dos comprueban el
cupo con la fila del salón bloqueada.

**La base no sólo responde a lo que se le pregunta: también avisa de que algo
cambió.** Publicar una tabla no amplía lo que una sesión puede leer —quién ve qué
contenido lo sigue decidiendo la RLS— pero sí reparte la noticia del cambio, y
eso tiene un límite medido que forma parte del sistema y no del cliente: **los
borrados no se filtran**, porque cuando llega el evento la fila ya no existe y no
hay contra qué evaluar la política.

Lo que todavía no cubre, y en los dos casos a propósito: `achievements` registra
los logros concedidos pero no existe el catálogo que enumere los posibles, y no
hay ninguna tabla que registre el **cumplimiento** de una misión, porque decidir
qué reporta el juego y con qué garantía es una decisión previa al contrato de
integración.

**Lo que el esquema deriva del correo cede ante el formato, nunca al revés.** El
disparador que crea el perfil acota el nombre de usuario que saca del correo y lo
deja vacío si no encaja, en lugar de rechazar el alta: las restricciones de
formato son del dominio y no se relajan para acomodar una derivación.

Las políticas de salones están **verificadas con sesiones reales de tutor y de
niño**, casos negativos incluidos: las once comprobaciones que lista
`docs/CONTEXT.md` §2.7, cerradas en el paso 11, más las trece del canje de
invitaciones. Lo único que queda fuera siguen siendo las **carreras**: el
`for update` del cupo y el de la invitación se probaron funcionalmente, no bajo
concurrencia.

*(Este párrafo decía que nada de lo que depende del rol o de la pertenencia se
había observado todavía. Llevaba dos pasos caducado y se corrige aquí; no es una
consecuencia del cambio que trajo el acceso con Google.)*

Las escrituras del rol también están medidas contra la base real: la función que
lo fija rechaza tanto si el rol ya se declaró como si el perfil tiene lazos de
salón, y en los dos casos se comprobó que **no hay escritura de ninguna clase**,
no sólo que el valor final coincide.
