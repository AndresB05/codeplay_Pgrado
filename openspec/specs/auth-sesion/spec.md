...
ejemplo cuando faltan o valen cero. Cuando algo falta y hay que replegarse, el
tratamiento genérico es **uno por rol** y nunca uno por pantalla.

Del perfil, **el nombre además se cambia**, desde la misma pantalla de Ajustes y
por las dos clases de cuenta: es lo que distingue a una persona de otra dentro de
un salón, y quedaba fijado en el registro para siempre. Cambiarlo se ve **de
inmediato en todas las superficies que lo enseñan**, que es parte del trato y no
un detalle: una pantalla que siguiera mostrando el nombre viejo le diría a quien
lo cambió que no se guardó.

**Y el rol se fija en el primer registro y no cambia nunca** —al contrario que el
nombre, y los dos son campos del mismo perfil—, que hoy es una propiedad
definitoria de esta capacidad y no un detalle de implementación: el rol es lo que
decide a qué panel se entra y qué se puede escribir, así que cambiarlo deja una
cuenta sin sitio. Enlazar varios proveedores sobre la misma cuenta sí es
