...
en su pantalla de cuenta. Desde que el juego escribe progreso, el número que se
enseña es el que el servidor concedió; el cero de una cuenta sin actividad se
muestra tal cual, porque la capacidad enseña lo que hay y no una cifra de
ejemplo.

**Cómo se llama un mundo es parte de esta capacidad.** El nombre y la descripción
tienen que decir lo que el niño va a hacer dentro, y ser los mismos en toda la
plataforma —la landing anterior al login incluida—, porque un mundo que promete
lo que el juego no permite es contenido que miente.
